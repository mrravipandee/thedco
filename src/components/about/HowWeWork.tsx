...
export function HowWeWork() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Pinned scroll behavior only on desktop
      mm.add("(min-width: 1024px)", () => {
        const trigger = ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            const index = Math.min(Math.floor(progress * PRINCIPLES.length), PRINCIPLES.length - 1);
  ...
  return (
    <section
      ref={containerRef}
      className="bg-black text-white relative border-b border-white/5"
    >
      {/* Desktop Pinned Layout */}
      <div className="hidden lg:block">
        <div className="max-w-7xl mx-auto px-6 md:px-12 h-screen flex items-center">
          <div className="grid grid-cols-12 gap-16 w-full items-center">
            ...
            <div className="col-span-5 space-y-8">
              ...
              <h2 className="text-4xl md:text-5xl font-serif text-white tracking-tight uppercase leading-tight">
                How We Work
              </h2>
            ...
            <div className="col-span-7 space-y-12">
              {PRINCIPLES.map((item, idx) => {
                ...
                return (
                  <div
                    key={item.num}
                    className={`transition-all duration-500 ease-in-out border-b border-white/5 pb-6 ${
                  ...
                  >
                    <div className="flex items-start space-x-6">
                      ...
                      <div className="space-y-2 flex-grow">
                        <h3
                          className={`text-xl md:text-2xl font-serif uppercase tracking-wider transition-colors duration-500 ${
                        ...
                        >
                          <div className="overflow-hidden">
                            <p className="text-sm md:text-base text-white/60 font-sans">
                              {item.desc}
                            </p>
                          </div>
                        </div>
      ...
      {/* Mobile Layout */}
      <div className="lg:hidden block py-24">
        <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-12">
          <div className="space-y-4">
            <span className="text-xs uppercase tracking-[0.3em] text-primary block font-semibold">
              OUR PHILOSOPHY
            </span>
            <h2 className="text-3xl md:text-4xl font-serif text-white tracking-tight uppercase">
              How We Work
            </h2>
          </div>
          ...
          <div className="space-y-12">
            {PRINCIPLES.map((item) => (
              <div
                key={item.num}
                className="mobile-row border-b border-white/5 pb-6 flex flex-col space-y-3"
              >
                <div className="flex items-center space-x-4">
                  <span className="text-lg font-serif text-primary">{item.num}</span>
                  <h3 className="text-lg md:text-xl font-serif uppercase tracking-wider text-white">
                    {item.title}
                  </h3>
                </div>
                <p className="text-sm text-white/60 font-sans max-w-xl pl-8 leading-relaxed">
                  {item.desc}
                </p>
              </div>
            ))}
